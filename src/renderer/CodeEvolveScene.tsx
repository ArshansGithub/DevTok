...
export const CodeEvolveScene: React.FC<{
    ...
    // Crossfade at ~45% of duration
    const midpoint = durationInFrames * 0.45;
    const transitionLen = 10;

    const fromOpacity = interpolate(
        frame, [midpoint - transitionLen / 2, midpoint + transitionLen / 2], [1, 0],
        { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' },
    );
    const toOpacity = interpolate(
        frame, [midpoint - transitionLen / 2, midpoint + transitionLen / 2], [0, 1],
        { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' },
    );
                ...
                <div style={{ position: 'relative', padding: '24px 24px', minHeight: 80 }}>
                    <div
                        style={{
                            ...
                            fontFamily: STYLE.fonts.mono, fontSize: 20, lineHeight: 1.6,
                            color: STYLE.colors.text, whiteSpace: 'pre-wrap', overflowWrap: 'break-word',
                            opacity: fromOpacity,
                        }}
                    >
                        {fromHtml ? <div dangerouslySetInnerHTML={{ __html: fromHtml }} /> : <code>{data.from}</code>}
                    </div>
                    <div
                        style={{
                            position: 'relative',
                    ...
                    >
                        {toHtml ? <div dangerouslySetInnerHTML={{ __html: toHtml }} /> : <code>{data.to}</code>}
                    </div>
                </div>
